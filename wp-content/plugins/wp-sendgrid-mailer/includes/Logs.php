    ...
    private function prepare_where_condition()
    {
        ...
        if($_POST)
        {
            if(isset($_POST['service']) && !empty($_POST['service']))
                $where_query .= " email_service = '{$_POST['service']}' and";

            if(isset($_POST['status']) && !empty($_POST['status']))
                $where_query .= " status = '{$_POST['status']}' and";
        ...
        switch( $column_name ) {
            ...
            case 'status':
                if($item->$column_name == 'Failed') {
                    echo "<span class = 'label label-danger'> Failed </span>  <span class='dashicons dashicons-info' data-toggle='tooltip' data-placement='bottom' title = '{$item->message}'></span>";
                }
